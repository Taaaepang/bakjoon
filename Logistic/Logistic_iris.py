...
def one_hot_encoding(Y):                                # class는 0, 1, 2 등 class에 해당되는 숫자로 표현되어있다.
    ...
    return num                                          # 0 = [1, 0, 0], 1 = [0, 1, 0] 2 = [0, 0, 1] 처럼 변환한다.
                                                        # 모든 class에 대해 변환한다.
...
X = iris.data                                           # Iris 데이터 받기
y = iris.target
y_name = iris.target_names


sample = np.random.randint(0, y.shape[0], 120)          # 데이터 셋 중 120개의 train 할 데이터를 샘플링한다.
x_train = list()
y_train = list()
x_test = list()
y_test = list()
...
sample = np.random.randint(0, y.shape[0], 30)           # 데이터 셋 중 30개의 test 할 데이터를 샘플링한다.
for i in sample:
    x_test.append(X[i])
    y_test.append(y[i])
...
Y_train = one_hot_encoding(y_train)                     # 각 데이터 셋에 대한 bias term추가, weight 초기값 생성,
X_train = Bias_term(x_train)                            # one hot encoding을 한다.
weight = weight_init(X_train, Y_train)
...
if idx == "single" or idx == "Single":
    # single class 식별
    select = int(input("class(0~2) ?= "))       # 어떤 class를 식별할 것인지 입력받는다.
    ...
    Logistic_single = Logistic_regression(X_train, Y_train[:, select], weight[:, select], learning_rate, idx)
    ...
    print("class = ", select)
    Logistic_single.predict(X_test, Y_test)     # test 데이터 셋과, 선택한 class의 실제 y값을 매개변수로 하여,
    ...
    Logistic_multi = Logistic_regression(X_train, Y_train, weight, learning_rate)
    for i in range(epoch):                      # epoch만큼 학습 반복 및 손실 값 출력.
        Logistic_multi.learn()
        cost.append(Logistic_multi.cost_function())     # 그래프를 그리기위해 손실값을 리스트에 삽입.
        print("epoch = ", i+1, "cost = ", cost[i])
    Logistic_multi.predict(X_test, Y_test)      # test 데이터 셋과, class의 실제 y값을 매개변수로 하여,
